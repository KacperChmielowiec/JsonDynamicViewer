...
{
    class AddFurtherTransformationModuleWindow : ChildWindow
    {
        public FurtherTransformationModule Result { get; private set; } = null;
        private List<(Button, FurtherTransformationModule)> moduleButtons = new List<(Button, FurtherTransformationModule)>();

        public AddFurtherTransformationModuleWindow(List<FurtherTransformationModule> currentModules)
        {
            this.Title = "Add Further transformation module...";
            this.Width = 410;
            this.Height = 450;
            ...
            header.Children.Add(new DPIAwareBox(Icons.GetIcon32("TreeViewer.Assets.AddFurtherTransformation")) { Width = 32, Height = 32, Margin = new Avalonia.Thickness(0, 0, 5, 0) });
            {
                TextBlock blk = new TextBlock() { Text = "Add further transformation", FontSize = 16, Foreground = new SolidColorBrush(Color.FromRgb(0, 114, 178)), VerticalAlignment = Avalonia.Layout.VerticalAlignment.Center, TextWrapping = TextWrapping.Wrap };
            ...
            foreach (FurtherTransformationModule module in from el in Modules.FurtherTransformationModules orderby el.Name ascending select el)
            {
                ...
                if (module.Repeatable || !currentModules.Contains(module))
                {
                    button.Click += (s, e) =>
                    {
                        Result = module;
                ...
                else
                {
                    ...
                    TextBlock blk = new TextBlock() { Text = "This module can only be used once!", TextWrapping = Avalonia.Media.TextWrapping.Wrap, IsHitTestVisible = false, FontWeight = Avalonia.Media.FontWeight.Bold, Foreground = new SolidColorBrush(Color.FromArgb(128, 234, 28, 36)), FontSize = 13 };
                    Grid.SetRow(blk, 2);
                    Grid.SetColumn(blk, 1);
                    Grid.SetColumnSpan(blk, 2);
                    content.Children.Add(blk);
            ...
            filterBox.KeyDown += (s, e) =>
            {
                if (e.Key == Avalonia.Input.Key.Enter)
                {
                    for (int i = 0; i < moduleButtons.Count; i++)
                    {
                        if (moduleButtons[i].Item1.IsVisible && (moduleButtons[i].Item2.Repeatable || !currentModules.Contains(moduleButtons[i].Item2)))
                        {
